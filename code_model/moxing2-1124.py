...
def Obj(model):
    return  sum(model.bia[i] * model.dvi[i] * model.Ni[i] for i in model.I) - \
           ...
           sum(model.xkjer[k, j, e, r] * model.Cer[e,r]  * model.dkjer[k, j, e,r] for e in model.E for r in model.R for j in model.J for k in model.K) - \
...
########公路运输转铁路运输的情况 
## 路途运输时间   
def a24(model, k, j, i, r):
    return model.tkjir[k, j, i, r] == model.dkjir[k, j, i, r] / model.vir[i, r] 
model.a24 = Constraint(model.K, model.J, model.I, model.R, rule=a24)

## 空车定位时间   
def a25(model, k, j, i):
    return model.tkji[k, j, i] == model.d0kji[k, j, i] / v0 
model.a25 = Constraint(model.K, model.J, model.I, rule=a25)
...
def a29_1(model, k, j, i): 
    return  model.Zkji[k, j, i] >= model.tckji[k, j, i] 
model.a29_1 = Constraint(model.K, model.J, model.I, rule=a29_1)

def a29_2(model, k, j, i): 
    return  model.tckji[k, j, i] >= model.Zkji[k, j, i] - LargeNumber *  (1 - model.mkji[k, j, i]) 
model.a29_2 = Constraint(model.K, model.J, model.I, rule=a29_2)

# todo 有问题这里？  for j in model.J for k in model.K for i in model.I
...
model.a34_1 = Constraint(model.I, rule=a34_1)
...
############铁路运输转公路运输的情况
## 路途运输时间   
def a35(model, k, j, e, r):
    return model.tkjer[k, j, e, r] == model.dkjer[k, j, e, r] / model.ver[e, r] 
model.a35 = Constraint(model.K, model.J, model.E, model.R, rule=a35)

## 空车定位时间   
def a36(model, k, j, e):
    return model.tkje[k, j, e] == model.d0kje[k, j, e] / v0 
model.a36 = Constraint(model.K, model.J, model.E, rule=a36)
...
instance = model.create_instance("./Data/moxing2-1124.dat")
